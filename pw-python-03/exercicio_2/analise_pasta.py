...
def faz_grafico_queijos( dados):
    chave = [key for x in dados.keys()]
    valores_Quantidades = [dados[x]["Quantidade"] for x in dados.keys()]
    plt.pie(valores_Quantidades, labels=chave, autopct='%1.0f%%')
    plt.title("Quantidades")
    plt.show()
    listagem_Tamanhos = [dados[i]["Size"] for i in dados.i()]
    plt.pie(listagem_Tamanhos, labels=chave, autopct="%1.0f%%")
    plt.title("Tamanhos")
    plt.show()
